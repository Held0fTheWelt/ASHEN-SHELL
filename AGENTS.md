...
4. **Language:** Same canonical policy as [`docs/dev/contributing.md`](docs/dev/contributing.md#repository-language). Hub files add **procedure-only scope** in `despaghettify/README.md` (**Language** paragraph) and each task Markdown `**Language:**` line — **links upward**, no restated project policy.

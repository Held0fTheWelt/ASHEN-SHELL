...
**Summary:** Delivers **Narrative Governance & Revision Foundation MVP** closure (promotion/rollback honesty, notifications, permissions, tests, acceptance matrix), the first **operational settings and runtime governance** slice (persistence, secrets, admin APIs, operator UI, world-engine resolved-config client, **docker-up** wiring, ops doc), **Docify** documentation-suite additions and audit/CI refinements that were previously noted only under **[Unreleased]**, **Contractify** as a new **fy** hub for contract discovery, anchoring vs projections, drift JSON, **CG-*** governance backlog, plus a **completion/hardening** slice (deterministic conflicts, operational versioning parses, bounded relation extensions, curated discovery widenings, committed JSON examples, hermetic **pytest** fixtures), and repository hygiene (**duplicate Despaghettify** root tree removal). **Alembic 043** is safe on **Docker/SQLite** when governance tables already exist or when **`bootstrap_presets`** must be seeded with timestamps.
...
- **Contractify hub tools:** `python -m pytest "'fy'-suites/contractify/tools/tests" -q` (from repo root, after **`pip install -e .`**) — sixteen tests; local audit JSON under **`'fy'-suites/contractify/reports/*.json`** is gitignored to avoid committing machine-generated drift snapshots.

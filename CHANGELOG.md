...
- **Role foundation (Phase 2):** Role model and `roles` table with seeded names: user, moderator, editor, admin. User model now has `role_id` FK to Role; `role` property returns role name for compatibility. New users get default role user. Centralized helpers: `user.has_role(name)`, `user.has_any_role(names)`, `user.is_admin`, `user.is_moderator_or_admin`; API permissions use `user.is_admin`; web guard `require_web_admin` added. Migration 007: create roles, backfill from `users.role`, drop string column. `ensure_roles_seeded()` on init-db and in tests; user_service and run.py seed-dev-user use role_id.

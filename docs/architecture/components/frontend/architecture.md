...
- `_compute_opening_shape_subgates` in `world-engine/world_engine/story_runtime/manager/` — Langfuse evidence; checks that indices 0–2 are `narrator` **block types** (subgate names `narrator_intro_present`, `role_anchor_present`, `scene_setup_present` are **historical labels**, not `narration_beat` values to write onto blocks).

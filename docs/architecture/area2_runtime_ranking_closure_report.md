...
|------|----------------|-----------------|--------|
| **G-CANON-RANK-01** | `RuntimeStageId.ranking` exists; staged execution orders signal → ranking → synthesis when synthesis runs | Ranking not canonical in identity or pipeline | **PASS** (`test_g_canon_rank_01_*`) |
| **G-CANON-RANK-02** | Distinct contracts and routes: signal vs `RankingStageOutput` vs synthesis `RoutingRequest` | Semantic collapse or wrong phase/task_kind | **PASS** (`test_g_canon_rank_02_semantic_boundary_gate`) |
| **G-CANON-RANK-03** | Ranking in traces, summary, rollup `ranking_context`, audit timeline | Ranking only in deep traces | **PASS** (`test_g_canon_rank_03_canonical_visibility_gate`) |
| **G-CANON-RANK-04** | `audit_summary` and `legibility.runtime_ranking_summary` both mirror orchestration ranking keys | Operator must infer ranking from synthesis alone | **PASS** (`test_g_canon_rank_04_operator_equality_gate` + cross-surface / G-CONV tests) |
| **G-CANON-RANK-05** | SLM-only, ranked-skip, ranked-then-LLM, degraded parse paths retain full canonical ranking surfaces | Important path treats ranking as implicit | **PASS** (`test_g_canon_rank_05_no_implicit_downgrade_gate`) |
| **G-CANON-RANK-06** | `RUNTIME_STAGED_REQUIRED` includes ranking; closure docs reference `build_ranking_routing_request`; healthy bootstrap routes ranking tuple | Inventory or startup truth omits ranking | **PASS** (`test_g_canon_rank_06_inventory_startup_truth_gate` + `test_g_conv_02_healthy_bootstrap_gate_runtime_specs`) |
| **G-CANON-RANK-07** | Architecture docs + this report reference Task 1B / G-CANON-RANK and binding interpretation | Documentation drift | **PASS** (`test_g_canon_rank_07_documentation_truth_gate`) |
| **G-CANON-RANK-08** | Ranked-skip and ranked-then-LLM complete with `success` and `guard_outcome` | Authority regression on ranking paths | **PASS** (`test_g_canon_rank_08_authority_guard_safety_gate`) |
...
- **Healthy bootstrap:** `test_g_conv_02_healthy_bootstrap_gate_runtime_specs` calls `route_model(build_ranking_routing_request(...))` alongside preflight, signal, and synthesis.

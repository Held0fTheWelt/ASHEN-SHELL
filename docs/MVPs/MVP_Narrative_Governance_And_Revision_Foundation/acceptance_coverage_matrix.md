...
| Preview sessions isolated from active sessions | `world-engine/app/narrative/preview_isolation.py`, `world-engine/app/api/http.py` internal narrative preview endpoints | `world-engine/tests/test_narrative_governance_api.py` |

## Package lifecycle

| Criterion | Implementation | Tests |
|---|---|---|
| Preview build produces isolated artifact registration | `backend/app/services/narrative_governance_service.py` (`build_preview`) | `backend/tests/test_narrative_governance_routes.py` |
...
| Rollback to older version without rebuild | `backend/app/services/narrative_governance_service.py` (`rollback_to_version`) | `backend/tests/test_narrative_governance_routes.py` |
| World-engine reload after package pointer change | `world-engine/app/api/http.py` (`/internal/narrative/packages/reload-active`) | `world-engine/tests/test_narrative_governance_api.py` |
...
| Workflow transitions enforced | `backend/app/services/narrative_governance_service.py` (`transition_revision`) | `backend/tests/test_narrative_governance_service.py`, `backend/tests/test_narrative_governance_routes.py` |
...
| Criterion | Implementation | Tests |
|---|---|---|
| Preview evaluation persisted against baseline | `backend/app/models/narrative_evaluation_run.py`, `backend/app/services/narrative_governance_service.py` | `backend/tests/test_narrative_governance_routes.py` |
| Promotion readiness derived from validation/evaluation/workflow | `backend/app/services/narrative_governance_service.py` run metadata and preview readiness payloads | `backend/tests/test_narrative_governance_routes.py` |
...
| Live quality metrics exposed (first-pass/retry/fallback) | `world-engine/app/narrative/runtime_health.py`, backend runtime health ingestion | `world-engine/tests/test_narrative_governance_api.py`, `backend/tests/test_narrative_governance_routes.py` |
...
| Criterion | Implementation | Tests |
|---|---|---|
| Rollback reachable from overview/packages journey | `administration-tool/templates/manage/narrative_governance/overview.html`, `packages.html` + package rollback API | manual smoke |
| Revisions page shows conflicts inline | `administration-tool/static/narrative_governance.js` (`loadRevisions`) | manual smoke + API tests |
| Package comparison readiness context available | `administration-tool/static/narrative_governance.js` combined package/runtime/evaluation payloads | manual smoke |
| Runtime health highlights spikes | `runtime_health.html` + health/fallback events APIs | manual smoke + API tests |
| Critical notifications visible without polling logs | `notifications.html` + notification feed/rules APIs | manual smoke + API tests |

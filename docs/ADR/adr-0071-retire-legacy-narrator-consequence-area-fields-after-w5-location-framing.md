...
`removal_ready` remains false because this ADR is still Proposed and a final
production-like dependency trace has not yet proven that no unsupported
downstream consumer depends on direct area-field presence.
...
- Static inventory cannot prove every external consumer has migrated; runtime
  and production-like evidence are still required before deletion.

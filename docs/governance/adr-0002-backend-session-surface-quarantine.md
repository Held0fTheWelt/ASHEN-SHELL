...
**No retirement or breaking URL removal** for any row in Appendix A without a separate **decision record** in this ADR (amendment) or in [audit_resolution_state_world_of_shadows.md](audit_resolution/audit_resolution_state_world_of_shadows.md) **decision log** (author, date, audience impact). Inventory and classification may proceed without that gate; **execute retire** only after the gate.
...
- State: [audit_resolution_state_world_of_shadows.md](audit_resolution/audit_resolution_state_world_of_shadows.md) (finding F-H2)

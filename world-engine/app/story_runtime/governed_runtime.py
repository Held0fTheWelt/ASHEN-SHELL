...
def _is_non_mock_model(registry: ModelRegistry, model_id: str | None) -> bool:
    if not model_id:
        return False
...
def _drama_profile_from_requirements(
    dramatic_requirements: dict[str, Any] | None,
    ...
    return req, high_complexity, "high_complexity" if high_complexity else "standard_complexity"


def _select_model_for_generation_mode(
    *,
    mode: str,
    high_complexity: bool,
    preferred_ai: str | None,
    fallback_ai: str | None,
    mock_mid: str | None,
    task_type: str,
    ...
    if mode == "ai_only":
        ...
        return selected, fallback_chain, True
    if mode == "mock_only":
        return mock_mid or "", None, False
    if high_complexity:
        return preferred_ai or fallback_ai or mock_mid or "", fallback_ai or mock_mid, False
    return fallback_ai or preferred_ai or mock_mid or "", preferred_ai or mock_mid, False


def _resolve_route_for_task(routes: dict[str, dict[str, Any]], task_type: str) -> dict[str, Any]:
...
class GovernedStoryRoutingPolicy:
    ...
    def choose(
        ...
        *,
        ...
        preferred_ai = preferred if _is_non_mock_model(self.registry, preferred) else None
        fallback_ai = fallback if _is_non_mock_model(self.registry, fallback) else None
        req, high_complexity, drama_profile = _drama_profile_from_requirements(dramatic_requirements)
        mode = (str(self.generation_mode or "").strip().lower() or "mock_only")
        selected, fallback_chain, mock_fallback_blocked = _select_model_for_generation_mode(
            mode=mode,
    ...
    def _resolve_route_for_task(self, task_type: str) -> dict[str, Any]:
        ...
        - Any other task_type: fail explicitly. No catch-all fallback over
          ``self.routes.values()``.
        """
        return _resolve_route_for_task(self.routes, task_type)

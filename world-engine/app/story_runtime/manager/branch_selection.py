...
class _BranchSelectionMixin:
    def _selectable_branch_tree_context(
        self,
        *,
        session_id: str,
        tree_id: str,
        node_id: str,
        trace_id: str | None,
    ) -> tuple[StorySession, dict[str, Any], dict[str, Any], dict[str, Any], list[dict[str, Any]], str, list[str]]:
        ...
        if not branch_tree_is_fresh(record, current_fingerprint):
            self._mark_branch_tree_stale_for_selection(
                session=session,
                record=record,
                tree_id=tree_id,
    ...
    def _mark_branch_tree_stale_for_selection(
        self,
        *,
    ...
    def _record_branch_selection_start(
        self,
        *,
        session: StorySession,
        tree_id: str,
    ...
    def _replay_selected_branch_path(
        self,
        *,
        session_id: str,
        ...
        for path_node in path_nodes:
            ...
            if not matched:
                ...
                break
        return replayed_turns, committed_events, replay_conflicts

    def _record_branch_replay_exception(
        self,
        *,
        session: StorySession,
        tree_id: str,
        ...
        self._append_branch_timeline_event(
            ...
            tree_id=tree_id,
            node_id=node_id,
            canonical_turn_id=_branch_selection_canonical_turn_id(committed_events[-1] if committed_events else {}),
    ...
    def _branch_selection_payload(
        ...
        *,
        ...
        return {
            ...
            "status": "branch_replay_conflict" if replay_conflicts else "committed",
            "tree_id": tree_id,
    ...
    def _commit_branch_selection_record(
        self,
        *,
        record: dict[str, Any],
        node_id: str,
        ...
        return self._persist_branching_tree_record(committed_record)

    def _record_branch_selection_final_event(
        self,
        *,
        session: StorySession,
        tree_id: str,
        ...
        *,
        ...
        with self._session_turn_lock(session_id):
            (
                ...
                selection_trace_id,
                selected_path_node_ids,
            ) = self._selectable_branch_tree_context(
                session_id=session_id,
                tree_id=tree_id,
                node_id=node_id,
                trace_id=trace_id,
            )
            self._record_branch_selection_start(
                session=session,
                tree_id=tree_id,
                node_id=node_id,
                node=node,
            ...
            replay_conflicts: list[dict[str, Any]] = []
            try:
                replayed_turns, committed_events, replay_conflicts = self._replay_selected_branch_path(
                    session_id=session_id,
                    path_nodes=path_nodes,
                    selection_trace_id=selection_trace_id,
                )
            except Exception as exc:
                self._record_branch_replay_exception(
                    session=session,
                    tree_id=tree_id,
                    node_id=node_id,
            ...
            selection = self._branch_selection_payload(
                tree_id=tree_id,
                node_id=node_id,
                node=node,
                selected_path_node_ids=selected_path_node_ids,
            ...
            committed_record = self._commit_branch_selection_record(
                record=record,
                node_id=node_id,
                selection=selection,
                after_fingerprint=after_fingerprint,
            )
            self._record_branch_selection_final_event(
                session=session,
                tree_id=tree_id,
                node_id=node_id,
                committed_events=committed_events,
...
def _branch_selection_canonical_turn_id(event: dict[str, Any]) -> str | None:
    if isinstance(event, dict) and event.get("canonical_turn_id"):
        return str(event.get("canonical_turn_id"))
    return None

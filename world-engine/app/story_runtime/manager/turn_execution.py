...
def _run_turn_graph_for_session(
    owner: Any,
    session: Any,
    *,
    player_input: str,
    ...
    graph_state = owner.turn_graph.run(
        ...
        prior_callback_web_state=owner._prior_callback_web_state_for_graph(session),
        prior_consequence_cascade_state=owner._prior_consequence_cascade_state_for_graph(session),
        ...
        actor_lane_context=owner._extract_actor_lane_context(session),
        session_input_language=session.session_input_language,
        session_output_language=session.session_output_language,
        story_runtime_experience=owner._story_runtime_experience_policy().effective,
        validation_execution_mode=owner._validation_execution_mode(),
    ...
    return graph_state, prior_ci


def _build_recoverable_graph_exception_event(
    session: Any,
    *,
    exc: Exception,
    player_input: str,
    ...
    return event, graph_state


def _persist_graph_exception_recovery(
    owner: Any,
    session: Any,
    *,
    exc: Exception,
    ...
    event, graph_state = _build_recoverable_graph_exception_event(
        session,
        exc=exc,
        player_input=player_input,
        trace_id=trace_id,
    ...
    return owner._persist_player_visible_turn_event(
        session=session,
...
class _TurnExecutionMixin:
    def _execute_turn_locked(self, *, session_id: str, player_input: str, trace_id: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            return _persist_graph_exception_recovery(
                self,
                session,
                exc=exc,
                player_input=player_input,

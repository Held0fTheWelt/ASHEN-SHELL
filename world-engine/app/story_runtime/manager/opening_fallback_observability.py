...
_OPENING_FALLBACK_REASON = "ldss_fallback_after_live_opening_failure"


def _record_primary_adapter_attempt(
    primary_metadata: dict[str, Any],
    metadata: dict[str, Any],
) -> None:
    prior_adapter = str(metadata.get("adapter") or "").strip()
    if not prior_adapter or prior_adapter in {"ldss_fallback", "ldss_deterministic"}:
        return
...
def _record_primary_routing_attempt(
    primary_metadata: dict[str, Any],
    graph_state: dict[str, Any],
) -> None:
    routing_state = graph_state.get("routing") if isinstance(graph_state.get("routing"), dict) else {}
...
def _record_primary_parser_evidence(
    primary_metadata: dict[str, Any],
    graph_state: dict[str, Any],
) -> None:
    ...
    for key in (
        ...
        if key in pae:
            primary_metadata[key] = pae[key]


def _record_self_correction_evidence(
    primary_metadata: dict[str, Any],
    graph_state: dict[str, Any],
) -> None:
    ...
    primary_metadata["capability_failure_before_retry"] = first_sc.get("capability_failure_before_retry")
    primary_metadata["self_correction_resolved_failure"] = any(
        bool(item.get("resolved_failure")) for item in sc_attempts if isinstance(item, dict)
    )
    primary_metadata["self_correction_model"] = last_sc.get("candidate_model")
    primary_metadata["self_correction_success"] = bool(last_sc.get("success")) and not last_sc.get(
        "parser_error"
    )


def _opening_primary_attempt_metadata(
    graph_state: dict[str, Any],
    metadata: dict[str, Any],
) -> dict[str, Any]:
    primary_metadata: dict[str, Any] = {}
    _record_primary_adapter_attempt(primary_metadata, metadata)
    _record_primary_routing_attempt(primary_metadata, graph_state)
    _record_primary_parser_evidence(primary_metadata, graph_state)
    _record_self_correction_evidence(primary_metadata, graph_state)
    return primary_metadata


def _opening_fallback_generation(
...
def _opening_prior_actor_lane(graph_state: dict[str, Any]) -> dict[str, Any]:
    prior_val = graph_state.get("validation_outcome") if isinstance(
        graph_state.get("validation_outcome"), dict
    ) else {}
    nested_lane = prior_val.get("actor_lane_validation")
    if isinstance(nested_lane, dict) and nested_lane:
        return dict(nested_lane)
    top_lane = graph_state.get("actor_lane_validation")
    if isinstance(top_lane, dict) and top_lane:
        return dict(top_lane)
    return {}


def _opening_fallback_validation(reason: str, prior_lane: dict[str, Any]) -> dict[str, Any]:
    validation: dict[str, Any] = {
        "status": "approved",
        "reason": _OPENING_FALLBACK_REASON,
        "validator_lane": "opening_fallback_policy_v1",
        "live_opening_failure_reason": reason,
    }
    if prior_lane:
        validation["actor_lane_validation"] = prior_lane
    return validation


def _opening_fallback_committed_result() -> dict[str, Any]:
    return {
        "commit_applied": True,
        "committed_effects": [
            {
...
def _opening_fallback_ledger(graph_state: dict[str, Any], reason: str) -> dict[str, Any]:
    ...
    fallback_ledger = set_aspect_record(
        ...
        make_aspect_record(
            ...
            reasons=[_OPENING_FALLBACK_REASON],
...
def _append_opening_fallback_degradation(fallback: dict[str, Any], reason: str) -> None:
    ...
    if _OPENING_FALLBACK_REASON not in signals:
        signals.append(_OPENING_FALLBACK_REASON)
...
class _OpeningFallbackObservabilityMixin:
    def _ldss_opening_fallback_state(
        ...
        *,
        ...
        primary_metadata = _opening_primary_attempt_metadata(graph_state, metadata)
        fallback["force_ldss_scene_fallback"] = True
        fallback["generation"] = _opening_fallback_generation(
            generation,
            metadata,
            primary_metadata,
            reason,
        )
        prior_lane = _opening_prior_actor_lane(graph_state)
        fallback["validation_outcome"] = _opening_fallback_validation(reason, prior_lane)
        fallback["committed_result"] = _opening_fallback_committed_result()
        fallback["turn_aspect_ledger"] = _opening_fallback_ledger(graph_state, reason)
        _append_opening_fallback_degradation(fallback, reason)
        return fallback

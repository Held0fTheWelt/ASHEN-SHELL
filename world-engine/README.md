...
The play service can merge published backend-authored experiences at startup.

Set `GAME_CONTENT_SOURCE_URL` to the backend feed endpoint, for example:

```
http://127.0.0.1:5000/api/v1/game-content/templates
```

When configured, remote templates override builtins by id. Builtins remain the fallback when the feed is unavailable.

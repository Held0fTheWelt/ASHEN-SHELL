...
class _DiagnosticsApiMixin:
    ...
    def get_w5_langfuse_metadata(self, session_id: str) -> dict[str, Any]:
        # ADR-0068: the narrator strict-off rollback path is gone. The
        # location_changed signal is computed from typed W5 history;
        # transition_from_previous is never consulted by admin diagnostics.
        session = self.get_session(session_id)

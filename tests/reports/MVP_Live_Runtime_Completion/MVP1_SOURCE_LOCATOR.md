...
| world-engine runtime profiles | runtime profile resolver (NEW for MVP1) | `world-engine/app/runtime/profiles.py` | `resolve_runtime_profile()`, `validate_selected_player_role()`, `RuntimeProfile`, `RuntimeProfileError` | found |
...
| frontend static JS | frontend role selection submit handler | `frontend/templates/session_start.html` | MVP1 adds role selector to `session_start.html` form with JavaScript for annette/alain visibility; `play_shell.js` handles in-session rendering | found — role selection lives in `session_start.html` form |
...
| tests | MVP1 test modules | `world-engine/tests/test_mvp1_experience_identity.py`, `backend/tests/runtime/test_mvp1_session_identity.py`, `frontend/tests/test_mvp1_play_launcher.py` | MVP1 behavior-proving tests (41 world-engine + 13 backend + 7 frontend) | found |
| reports | MVP1 artifact reports | `tests/reports/MVP_Live_Runtime_Completion/` | `MVP1_SOURCE_LOCATOR.md`, `MVP1_OPERATIONAL_EVIDENCE.md`, `MVP1_HANDOFF_RUNTIME_PROFILE.md`, `MVP1_CAPABILITY_EVIDENCE.md` | found |
| ADRs | `docs/ADR/` | `docs/ADR/MVP_Live_Runtime_Completion/` | `adr-mvp1-001-experience-identity.md`, `adr-mvp1-002-runtime-profile-resolver.md`, `adr-mvp1-003-role-selection-actor-ownership.md`, `adr-mvp1-005-canonical-content-authority.md`, `adr-mvp1-006-evidence-gated-capabilities.md`, `adr-mvp1-016-operational-gates.md` | found |
| docker-up.py | `docker-up.py` | `docker-up.py` (repo root) | `main()` → `cmd_default()` → `_run()` → docker compose up -d --build | found |
| run-test.py | `run-test.py` | `run-test.py` (repo root) | MVP1 thin wrapper around `tests/run_tests.py` that maps `--unit/--integration/--e2e/--all/--mvp1` to `--suite` flags. Maps `--mvp1` to `--suite engine backend` to run world-engine and backend MVP1 tests. | found |
| GitHub workflows | `.github/workflows/*.yml` | `.github/workflows/engine-tests.yml`, `.github/workflows/backend-tests.yml`, `.github/workflows/mvp1-tests.yml` | `mvp1-tests.yml` runs dedicated MVP1 tests for world-engine, backend, frontend, and tooling gate verification. | found |

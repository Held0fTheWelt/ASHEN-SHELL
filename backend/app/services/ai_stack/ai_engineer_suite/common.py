...
_RUNTIME_PRESETS = [
    ...
    },
    {
        ...
        "display_name": "Cost Aware (Local)",
        "category": "runtime_stack",
        "description": "Local-first cost-optimised preset for Ollama and other local providers.",
        "stability": "recommended",
        "is_local_only": True,
        "impact_summary": [
            "Routes generation to local providers (Ollama) via routed_llm_slm.",
            "Uses sparse-only retrieval to avoid embedding costs.",
            "Keeps retries minimal to reduce local model load.",
        ],
        "compatibility_notes": [
            "Requires a local provider (e.g. Ollama) with enabled models and routes.",
            "Falls back to mock if no local non-mock provider is reachable.",
        ],
        "controlled_values": {
            "generation_execution_mode": "routed_llm_slm",
            "validation_execution_mode": "schema_only",
            "provider_selection_mode": "local_only",
            "runtime_profile": "cost_aware",

"""Single owned surface: GoC runtime scene_id → scene_guidance.yaml phase keys.

See ADR-0003: the mapping here is the **source of truth**; tests verify consistency
against ``scene_guidance.yaml`` and ``scenes.yaml`` phase ids. Do not duplicate this
mapping elsewhere (CI enforces via ``tools/verify_goc_scene_identity_single_source.py``).
"""
...
def all_expected_guidance_phase_keys() -> frozenset[str]:
    """Phase block keys that must exist in merged scene_guidance.yaml for GoC."""

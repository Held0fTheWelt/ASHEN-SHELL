...
# Wave 5: assembled from runtime_executor SOURCE_LINES via tools/architecture_assurance/assemble_runtime_executor.py
...
class RuntimeTurnGraphExecutor:
    ...
    def _build_graph(self):
        ...
        graph.add_edge("render_visible", "translate_output")
        graph.add_edge("translate_output", "package_output")
    ...
    def _translation_adapter_candidate(self) -> tuple[str, str, BaseModelAdapter | None, str | None]:
        try:
            decision = self.routing.choose(task_type="translation")
        except Exception:
            decision = None
    ...
    def _interpret_input(self, state: RuntimeTurnState) -> RuntimeTurnState:
        ...
        input_kind_map = STRUCTURAL_KIND_TO_INPUT_ROUTING
        ...
        if kind_raw in ("explicit_command", "meta"):
            ...
            if semantic_action:
                ...
                if silence_negative_space_active:
                    ...
                    pik = STRUCTURAL_KIND_TO_INTENT_FALLBACK.get(kind_raw, "speech")
                intent_fields["player_input_kind"] = pik
    ...
    def _resolve_player_action(self, state: RuntimeTurnState) -> RuntimeTurnState:
        ...
        cached_surface = (
            translation_shell.get("interaction_surface")
            if isinstance(translation_shell.get("interaction_surface"), dict)
            else None
        )
        resolution = resolve_player_action(
            ...
            interaction_surface=cached_surface,
        )
    ...
    def _realize_via_capabilities(self, state: RuntimeTurnState) -> RuntimeTurnState:
        ...
        if capability == CAPABILITY_NARRATOR_LOCATION_TRANSITION:
            prompt_lines.append(
                ...
                "Name the destination by its in-world label. "
    ...
    def _derive_sensory_context(self, state: RuntimeTurnState) -> RuntimeTurnState:
        ...
        result = derive_sensory_context(
            ...
            w5_location_framing=state.get("w5_location_framing")
            if isinstance(state.get("w5_location_framing"), dict)
            else None,
    ...
    def _self_correct_generation(
        ...
        if retry_context_synthesis_prompt.strip():
            # E7: attach a truncated synthesis hint only — do not grow the base prompt.
            hint = retry_context_synthesis_prompt.strip()
            if len(hint) > 500:
                hint = hint[:500] + "…"
            base = (retry_prompt or str(state.get("player_input") or "")).strip()
            if len(base) > 1500:
                base = base[:1500] + "…"
            retry_prompt = f"{base}\n\n{hint}".strip()

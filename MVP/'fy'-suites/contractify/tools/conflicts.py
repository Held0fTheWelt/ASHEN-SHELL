...
def _iter_adr_files(repo: Path) -> list[Path]:
    out: list[Path] = []
    seen: set[str] = set()
    for rel_dir in ("docs/ADR", "docs/governance"):
        d = repo / rel_dir
        if not d.is_dir():
            continue
        for adr in sorted(d.glob("adr-*.md")):
...
def _ownership_map(repo: Path) -> dict[str, dict]:
    path = repo / "validation/fy_inputs/contractify_vocabulary_ownership.json"
    if not path.is_file():
        return {}
    try:
...
def detect_adr_vocabulary_overlap(repo: Path) -> list[ConflictFinding]:
    ...
    adr_files = _iter_adr_files(repo)
    if not adr_files:
        return []
    ownership = _ownership_map(repo)
    out: list[ConflictFinding] = []
    for bucket, keywords in _ADR_OVERLAP_TERMS:
        ...
        if len(hits) >= 2:
            if _governed_runtime_spine_overlap(bucket, hits):
                continue
            rule = ownership.get(bucket) or ownership.get(bucket.lower())
            if isinstance(rule, dict):
                owner = Path(str(rule.get("owner_anchor", ""))).name
                allowed = _basename_set([str(x) for x in rule.get("allowed_dependent_mentions", []) if isinstance(x, str)])
                allowed.add(owner)
                if owner and owner in hits and set(hits).issubset(allowed):
                    continue
            out.append(
                ConflictFinding(
...
def detect_all_conflicts(
    ...
    *,
    ...
    # Curated runtime/MVP unresolved areas are emitted separately by audit_pipeline as manual_unresolved_areas.
    # Do not duplicate them into the generic conflict list here.
    _curated_contracts, _curated_projections, _curated_relations, _curated_conflicts, _curated_families = build_runtime_mvp_spine(repo)
    seen: set[str] = set()

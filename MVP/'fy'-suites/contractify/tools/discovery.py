...
def _preferred_adr_dir(repo: Path) -> Path:
    canonical = repo / "docs" / "ADR"
    if canonical.is_dir():
        return canonical
    return repo / "docs" / "governance"
...
def discover_contracts_and_projections(
    ...
    *,
    ...
    if p_openapi.is_file():
        ...
        add(
            _contract(
                ...
                summary="Bounded machine-readable schema for the Flask backend /api/v1 inventory; subordinate to higher-order runtime/API authority for overlapping run semantics.",
                ...
                source_of_truth=True,
                confidence=conf,
                discovery_reason="A: canonical OpenAPI document under docs/api/ (bounded backend API schema, not universal runtime authority)",
                tags=["http", "openapi", "backend", "projection-governance"],
                implemented_by=impl,
                documented_in=doc_in,
                projected_as=proj_as,
                notes="Authority is bounded to the packaged backend /api/v1 schema surface. Higher-order runtime/API authority still lives in the canonical runtime contract family.",
            )
        )
    ...
    # A3 — ADRs
    adr_dir = _preferred_adr_dir(repo)
    if adr_dir.is_dir():
        for adr in sorted(adr_dir.glob("adr-*.md")):
            ...
            add(
                _contract(
                    ...
                    discovery_reason="A: ADR filename pattern under docs/ADR/ (fallback docs/governance/)",

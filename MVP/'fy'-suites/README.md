...
1. **Lean v24 package:** prefer `python scripts/run_fy_governance_cycle.py` or explicit `PYTHONPATH`-based suite calls. The full historical root `pyproject.toml` is intentionally not restored in this curated package.
...
2. If a future fuller repository package reintroduces editable-install support, register the suite in the restored root packaging metadata. This lean v24 package intentionally omits that broader bootstrap layer.

...
def get_story_manager(request: Request) -> Any:
    manager = getattr(request.app.state, "story_manager", None)
    if manager is None:
        reason = getattr(request.app.state, "story_manager_error", None)
        detail = "Story runtime unavailable"
        if isinstance(reason, str) and reason.strip():
            detail = f"Story runtime unavailable: {reason}"
...
def create_story_session(payload: CreateStorySessionRequest, manager: Any = Depends(get_story_manager)) -> dict[str, Any]:
...
def execute_story_turn(
    ...
    request: Request,
    manager: Any = Depends(get_story_manager),
) -> dict[str, Any]:
...
def get_story_state(session_id: str, manager: Any = Depends(get_story_manager)) -> dict[str, Any]:
    try:
...
def get_story_diagnostics(session_id: str, manager: Any = Depends(get_story_manager)) -> dict[str, Any]:
    try:

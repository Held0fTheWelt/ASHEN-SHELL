...
class RuntimeManager:
    def __init__(
        ...
        *,
        ...
        self.builtin_templates: dict[str, ExperienceTemplate] = load_builtin_templates()
        self.templates: dict[str, ExperienceTemplate] = dict(self.builtin_templates)
    ...
    def sync_templates(self, *, force: bool = False) -> None:
        ...
        active_remote_ids = set(remote_templates)
        stale_remote_ids = {
            template_id
            for template_id, source in self.template_sources.items()
            if source == "backend_published" and template_id not in active_remote_ids
        }
        for template_id in stale_remote_ids:
            builtin_template = self.builtin_templates.get(template_id)
            if builtin_template is not None:
                self.templates[template_id] = builtin_template
                self.template_sources[template_id] = "builtin"
            else:
                self.templates.pop(template_id, None)
                self.template_sources.pop(template_id, None)
    ...
    def _record_template_activation(self, instance: RuntimeInstance, template: ExperienceTemplate) -> None:
        source = self.template_sources.get(template.id, "builtin")
        instance.metadata.setdefault("template_activation_source", source)
    ...
    def _template_details_for_instance(self, instance: RuntimeInstance) -> tuple[str, dict[str, Any]]:
        metadata = instance.metadata if isinstance(instance.metadata, dict) else {}
        stored_source = metadata.get("template_activation_source")
        stored_template = metadata.get("template_activation_template")
        template = self.templates.get(instance.template_id)

        if isinstance(stored_template, dict):
            template_payload = {
                "id": stored_template.get("id") or instance.template_id,
                "title": stored_template.get("title") or instance.template_title,
                "kind": stored_template.get("kind") or instance.kind.value,
                "join_policy": stored_template.get("join_policy") or instance.join_policy.value,
                "min_humans_to_start": stored_template.get("min_humans_to_start", metadata.get("min_humans_to_start", 1)),
            }
            ...
            template_payload = {
                ...
                "title": instance.template_title,
                "kind": instance.kind.value,
                "join_policy": instance.join_policy.value,
                "min_humans_to_start": metadata.get("min_humans_to_start", 1),
            }

        source = stored_source if isinstance(stored_source, str) and stored_source.strip() else self.template_sources.get(instance.template_id, "builtin")
        return source, template_payload
    ...
    def _bootstrap_instance(
        ...
        self._record_template_activation(instance, template)
    ...
    def get_run_details(self, run_id: str) -> dict[str, Any]:
        instance = self.instances[run_id]
        template_source, template_payload = self._template_details_for_instance(instance)
        return {

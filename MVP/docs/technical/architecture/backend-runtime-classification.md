...
**Related:** [canonical_runtime_contract.md](./canonical_runtime_contract.md) (nested run V1, consumer/producer split); [../../../governance/V24_BACKEND_TRANSITIONAL_RETIREMENT_LEDGER.md](../../../governance/V24_BACKEND_TRANSITIONAL_RETIREMENT_LEDGER.md) (retirement decision log).
...
## Transitional surface inventory (current package retirement-governance view)

The table below is the bounded review surface for this package. It does **not** broaden the architecture. It only clarifies what the currently present backend transitional/session surfaces are allowed to do.

| Surface | Authority level | Current role | Retirement status | Retained reason right now | Evidence already present | Evidence required for next status transition |
|---|---|---|---|---|---|---|
| `backend/app/api/v1/session_routes.py` | non-authoritative | `compatibility_only`, `bridge_consumer`; may create warned volatile backend-local sessions and may bridge to world-engine for authoritative state/turns | **retirement-open** for the backend-local session-shaped path | compatibility/tests/MCP-shaped flows still use a session-shaped backend entrypoint | route warnings and bridge preference are covered in `backend/tests/test_session_routes.py`; additional bridge contract coverage exists in `backend/tests/test_session_api_contracts.py` | prove caller/test/operator migration away from backend-local session creation or reclassify route to bridge-only behavior; then update ADR-0002, this table, ledger, and Contractify together |
| `backend/app/runtime/session_store.py` | non-authoritative | `volatile_local_state`, `retained_for_tests`; process-local registry only | **retirement-open** | current backend-local compatibility flow and related tests still read/write the volatile registry | bounded CRUD/non-leakage evidence exists in `backend/tests/runtime/test_session_store.py`; source explicitly labels volatility/non-authority | prove no governed route/service/test/operator flow still depends on the local registry; then remove or reclassify without hiding diagnostics or compatibility dependencies |
| `backend/app/services/session_service.py` | non-authoritative | `compatibility_only`, `bridge_consumer`; local bootstrap helper only | **retirement-open** | POST `/api/v1/sessions` still relies on local bootstrap; tests still call `create_session` directly | source keeps deferred methods unimplemented; create-session existence is covered in `backend/tests/services/test_session_service.py` | prove POST `/api/v1/sessions` no longer needs local bootstrap or explicitly reclassify the helper as a minimal non-runtime bridge; callers/tests must migrate first |
| `backend/app/api/v1/world_engine_console_routes.py` | non-authoritative | `operator_support`, `bridge_consumer`; admin/JWT/capability-gated proxy to world-engine | **intentionally retained** in this package, not in the retirement-open trio | current package still needs governed admin/operator observation and operation against world-engine | source-level auth/capability and world-engine-only bridge semantics are present; focused source validation exists in `tests/smoke/test_backend_transitional_retirement_surface_contracts.py` | only reconsider retirement if a future package supplies equivalent governed admin/operator observability and operations elsewhere; direct route-behavior evidence should be strengthened before any such claim |

## Retirement claim rule

A future cycle may claim retirement of a backend transitional session surface only when all of the following are true:

1. the code is removed or explicitly reclassified in the package docs;
2. affected callers/tests/operator flows are migrated or intentionally preserved elsewhere;
3. the relevant ADR/runtime-classification/ledger entries are updated together; and
4. a re-audit verifies that no world-engine authority drift was introduced by the removal.

If those conditions are not met, the honest state is **quarantined and still present**, not retired.

## POST-locality final resolution note

For the current package state, the dominant backend transitional retirement residue is now:

- POST bootstrap locality, and
- explicit compatibility fallback for snapshot/state/diagnostics.

This is narrower than a broad backend-local session surface and should be re-audited as such.


## POST-locality final resolution note

The dominant remaining backend-local residue is now best read as explicit compatibility bootstrap on POST, with `session_service` tightly bounded to helper duty and `session_store` mostly derivative of that POST behavior.

# Postman collection alignment and projection status

> **Projection governance status:** `operational_projection`
>
> This document governs the **Postman-facing projection layer**. It is not a canonical API contract.
>
> Upstream authority order for Postman-facing artifacts:
>
> 1. `docs/technical/architecture/canonical_runtime_contract.md` for overlapping runtime/API payload semantics
> 2. `docs/api/openapi.yaml` for the bounded backend `/api/v1` machine schema shipped in this MVP
> 3. `postman/postmanify-manifest.json` for the last recorded OpenAPI → Postman generation fingerprint
> 4. generated Postman collections and this guide as convenience/operational projections

## Current position

- The old hand-maintained `WorldOfShadows_API.postman_collection.json` is **not** the current in-package authority model.
- The packaged Postman artifacts are **generated operational projections** rooted in `docs/api/openapi.yaml`.
- Coverage is limited to the **backend HTTP `/api/v1` inventory** represented in the current OpenAPI artifact.
- WebSocket/runtime live-play behavior is **not** governed here as Postman JSON truth.

## What this document is allowed to do

- explain how the packaged Postman assets relate to OpenAPI,
- mark historical manual guidance as legacy or partial,
- and point reviewers to generated collections and manifest state.

## What this document must not be treated as

- a higher-order runtime/API contract,
- a proof that every request body/example is current,
- or a license for Postman guidance to outrank `docs/api/openapi.yaml`.

## Packaged Postman artifacts

- `postman/WorldOfShadows_Complete_OpenAPI.postman_collection.json`
- `postman/suites/WorldOfShadows_Suite_*.postman_collection.json`
- `postman/postmanify-manifest.json`
- `postman/README.md`
- `postman/WEBSOCKET_MANUAL.md`

## Historical checklist residue

The remaining forum-oriented checklist below is kept as **historical implementation residue** and manual review aid.
It is no longer the primary source for Postman collection truth.

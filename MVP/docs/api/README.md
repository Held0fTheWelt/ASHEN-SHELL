...
## API projection governance status

This package treats the API-facing family as a **layered projection surface**, not as a second competing truth layer.

### Authority order for API-facing artifacts
...
2. **Bounded canonical API schema**
   - `docs/api/openapi.yaml` for the **Flask backend `/api/v1` route inventory** only
3. **Projection / convenience artifacts**
   - `docs/api/REFERENCE.md`
   - `docs/api/POSTMAN_COLLECTION.md`
   - `docs/dev/api/openapi-and-api-explorer-strategy.md`
   - `postman/README.md` and generated `postman/*.postman_collection.json`

### What this README is

This README is a **projection_doc** for navigation and operator/developer orientation.
It may summarize endpoints, hosts, and workflow pointers, but it **must not** be treated as a stronger authority than the canonical runtime contract family or the bounded OpenAPI schema.

### API projection boundary matrix

| Artifact | Role | Authority level | Upstream source-of-truth anchor | Coverage status | Forbidden interpretation |
|---|---|---|---|---|---|
| `docs/api/openapi.yaml` | `canonical_api_schema` | bounded canonical | `canonical_runtime_contract.md` for overlapping run payload semantics; runtime authority docs/ADR for authority ownership | backend `/api/v1` inventory present, schema depth still partial | not the full world-engine/API truth layer |
| `docs/api/README.md` | `projection_doc` | projection-only | `docs/api/openapi.yaml` + canonical runtime/API anchors | partial human overview | not a normative contract |
| `docs/api/REFERENCE.md` | `convenience_reference` | projection-only | `docs/api/openapi.yaml` + validated route code + canonical runtime contract where applicable | broad examples, but not complete proof of coverage | not canonical payload truth |
| `docs/api/POSTMAN_COLLECTION.md` | `operational_projection` | projection-only | `docs/api/openapi.yaml` + `postman/postmanify-manifest.json` | operational guide + historical notes | not a maintained source-of-truth schema |
| `docs/dev/api/openapi-and-api-explorer-strategy.md` | `planned_projection` | projection-only | same upstream anchors | roadmap/strategy only | not evidence that sync is fully solved |
| `postman/*.postman_collection.json` | `operational_projection` | generated convenience | `docs/api/openapi.yaml` via `postman/postmanify-manifest.json` | backend HTTP subset only; no WebSocket truth | not allowed to outrank OpenAPI or runtime contracts |

See also: `governance/V24_API_PROJECTION_GOVERNANCE_LEDGER.md`.
...
- Backend integration historical note was intentionally not carried in this lean package; see [Service boundaries](../technical/architecture/service-boundaries.md) for the current governed boundary view

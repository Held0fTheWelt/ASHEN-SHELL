...
def test_admin_session_evidence_requires_authoritative_id_or_explicit_fallback_for_unbound_session(client, moderator_headers):
    create_resp = client.post("/api/v1/sessions", json={"module_id": "god_of_carnage"})
    session_id = create_resp.get_json()["session_id"]
    response = client.get(f"/api/v1/admin/ai-stack/session-evidence/{session_id}", headers=moderator_headers)
    assert response.status_code == 409
    assert response.get_json()["error"] == "authoritative_world_engine_story_session_id_required"


def test_admin_session_evidence_can_still_use_explicit_local_fallback_for_unbound_session(client, moderator_headers, monkeypatch):
    create_resp = client.post("/api/v1/sessions", json={"module_id": "god_of_carnage"})
    session_id = create_resp.get_json()["session_id"]
    monkeypatch.setattr("app.services.ai_stack_evidence_service._latest_writers_room_review", lambda: None)
    monkeypatch.setattr("app.services.ai_stack_evidence_service._latest_improvement_package", lambda: None)
    response = client.get(f"/api/v1/admin/ai-stack/session-evidence/{session_id}?allow_backend_local_fallback=1", headers=moderator_headers)
    assert response.status_code == 200
    assert response.get_json()["backend_session_id"] == session_id

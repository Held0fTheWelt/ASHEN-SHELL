...
def test_game_run_detail_and_transcript_player_endpoints(client, auth_headers, monkeypatch):
    monkeypatch.setattr("app.api.v1.game_routes.get_play_run_details", lambda run_id: {"run": {"id": run_id}, "template_source": "backend_published", "template": {"id": "god_of_carnage_solo", "title": "God of Carnage", "kind": "solo_story", "join_policy": "solo", "min_humans_to_start": 1}, "store": {"kind": "memory"}, "lobby": {"status": "active"}})
    monkeypatch.setattr("app.api.v1.game_routes.get_play_run_transcript", lambda run_id: {"run_id": run_id, "entries": [{"text": "One line."}]})

    detail = client.get('/api/v1/game/runs/run-123', headers=auth_headers)
    assert detail.status_code == 200
    assert detail.get_json()['template_source'] == 'backend_published'

    transcript = client.get('/api/v1/game/runs/run-123/transcript', headers=auth_headers)
    assert transcript.status_code == 200
    assert transcript.get_json()['entries'][0]['text'] == 'One line.'


def test_game_run_detail_and_transcript_require_auth(client):
    assert client.get('/api/v1/game/runs/run-123').status_code == 401
    assert client.get('/api/v1/game/runs/run-123/transcript').status_code == 401

...
Exposed operations:
- create_local_bootstrap_session: start a **local** session from a module (registers in volatile store)
- create_session: start a **local** session from a module (compatibility alias; deprecated)
...
def create_local_bootstrap_session(module_id: str) -> SessionState:
    """Bootstrap in-process ``SessionState`` from a content module (compatibility bootstrap only).
...
def create_session(module_id: str) -> SessionState:
    """Deprecated compatibility alias for create_local_bootstrap_session()."""
    return create_local_bootstrap_session(module_id)

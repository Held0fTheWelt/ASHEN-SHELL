...
def admin_ai_stack_session_evidence(session_id: str):
    ...
    bundle = build_session_evidence_bundle(session_id=session_id, trace_id=trace_id, allow_backend_local_fallback=_allow_backend_local_fallback())
    ...
    if bundle.get("error") == "backend_session_not_found":
        status = 404
    elif bundle.get("error") == "authoritative_world_engine_story_session_id_required":
        status = 409
    else:
        status = 200

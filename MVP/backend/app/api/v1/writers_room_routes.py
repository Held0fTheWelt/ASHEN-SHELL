...
def _writers_room_services():
    """Load heavy Writers' Room services lazily.

    This keeps unrelated backend startup and bridge-facing integration tests from importing
    LangChain/LangGraph-heavy Writers' Room dependencies on cold/minimal environments.
    Route behavior is unchanged once the endpoint is actually invoked.
    """
    ...
    return {
        "apply_writers_room_decision": apply_writers_room_decision,
        "get_writers_room_review": get_writers_room_review,
        "run_writers_room_review": run_writers_room_review,
        "submit_writers_room_revision": submit_writers_room_revision,
    }
...
def create_writers_room_review():
    ...
    report = _writers_room_services()["run_writers_room_review"](
...
def get_writers_room_review_by_id(review_id: str):
    try:
        review = _writers_room_services()["get_writers_room_review"](review_id=review_id)
...
def submit_writers_room_review_revision(review_id: str):
    ...
    try:
        review = _writers_room_services()["submit_writers_room_revision"](
            review_id=review_id,
            actor_id=actor_id,
            focus=focus,
...
def set_writers_room_review_decision(review_id: str):
    ...
    trace_id = g.get("trace_id") or get_trace_id()
    try:
        review = _writers_room_services()["apply_writers_room_decision"](
            review_id=review_id,
            actor_id=actor_id,
            decision=decision,
            note=str(data.get("note") or "").strip(),

...
@api_v1_bp.route("/game/runs/<run_id>", methods=["GET"])
@limiter.limit("60 per minute")
def game_run_detail(run_id: str):
    try:
        _require_game_user()
...
@api_v1_bp.route("/game/runs/<run_id>/transcript", methods=["GET"])
@limiter.limit("60 per minute")
def game_run_transcript(run_id: str):
    try:
        _require_game_user()

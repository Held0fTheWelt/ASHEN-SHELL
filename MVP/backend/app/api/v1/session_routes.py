...
from app.services.session_service import (
    create_local_bootstrap_session,
    create_session,
    get_session,
    execute_turn,
    get_session_logs,
...
from app.runtime.runtime_models import SessionContextLayers, DegradedSessionState
...
def _allow_backend_local_fallback() -> bool:
    raw = (request.args.get("allow_backend_local_fallback") or "").strip().lower()
    return raw in {"1", "true", "yes", "on"}


def _explicit_fallback_required_response(*, session_id: str, surface: str):
    return jsonify({
        "session_id": session_id,
        "error": {
            "code": "AUTHORITATIVE_STORY_SESSION_ID_OR_EXPLICIT_LOCAL_FALLBACK_REQUIRED",
            "message": f"{surface} requires world_engine_story_session_id or allow_backend_local_fallback=1 while the compatibility session remains unbound.",
        },
        "warnings": [
            "unbound_backend_local_lookup_not_permitted_by_default_for_this_surface",
            "backend_local_fallback_can_only_be_used_as_explicit_compatibility_residue",
        ],
    }), 409


def _minimize_bound_compatibility_state(state) -> None:
    metadata = state.metadata if isinstance(state.metadata, dict) else {}
    metadata["compatibility_state_scope"] = "minimal_after_authoritative_binding"
    metadata["compatibility_state_minimized"] = True
    state.metadata = metadata
    state.canonical_state = {"compatibility_state_scope": "minimal_after_authoritative_binding"}
    state.context_layers = SessionContextLayers()
...
def create_new_session():
    ...
    try:
        session = create_local_bootstrap_session(module_id)
        warnings = [
        ...
        try:
            ...
            _minimize_bound_compatibility_state(session)
            warnings.extend([
                "world_engine_story_session_bootstrapped_for_bridge_first_compatibility",
                "backend_local_compatibility_state_minimized_after_authoritative_binding",
            ])
            ...
            warnings.append(f"world_engine_story_session_bootstrap_failed:{exc.status_code}")
            log_world_engine_bridge(
                trace_id,
                operation="create_story_session",
                backend_session_id=session.session_id,
        ...
        if isinstance(body.get("metadata"), dict):
            engine_story_session_id = body["metadata"].get("world_engine_story_session_id")
            if isinstance(engine_story_session_id, str) and engine_story_session_id.strip():
                body["world_engine_story_session_id"] = engine_story_session_id
...
def get_session_by_id(session_id):
    ...
    if not (isinstance(engine_story_session_id, str) and engine_story_session_id.strip()):
        if not _allow_backend_local_fallback():
            return _explicit_fallback_required_response(session_id=session_id, surface="snapshot")
        response["warnings"].append("backend_local_fallback_explicitly_requested")
    if isinstance(engine_story_session_id, str) and engine_story_session_id.strip():
        trace_id = g.get("trace_id") or get_trace_id()
        try:
...
def get_session_diagnostics(session_id):
    ...
    if not (isinstance(engine_story_session_id, str) and engine_story_session_id.strip()):
        if not _allow_backend_local_fallback():
            return _explicit_fallback_required_response(session_id=session_id, surface="state")
        response["warnings"].append("backend_local_fallback_explicitly_requested")
    if isinstance(engine_story_session_id, str) and engine_story_session_id.strip():
        trace_id = g.get("trace_id") or get_trace_id()
        try:

...
> **Projection governance status:** `convenience_reference`
>
> This document is a **human-readable projection** of the backend API surface.
> It is useful for examples, auth notes, and quick lookup, but it is **not** the highest-order authority for overlapping runtime/API semantics.
>
> Treat upstream authority in this order:
>
> 1. `docs/technical/architecture/canonical_runtime_contract.md` for overlapping play-service run lifecycle payload semantics.
> 2. `docs/api/openapi.yaml` for the bounded Flask backend `/api/v1` route inventory in this MVP package.
> 3. validated route code and focused tests.
>
> **Do not treat this file as:**
>
> - proof that every route/payload example is fully current,
> - a replacement for the canonical runtime contract family,
> - or a signal that Postman/OpenAPI/Markdown are all perfectly synchronized.
>
> Drift remains reviewable in `governance/V24_API_PROJECTION_GOVERNANCE_LEDGER.md`.

Complete reference for all REST API endpoints in the World of Shadows backend. For authentication details, see `../security/README.md`.
...
- [Development Setup](../development/LocalDevelopment.md)
- [Technical architecture](../technical/architecture/architecture-overview.md)
- [Service boundaries](../technical/architecture/service-boundaries.md)

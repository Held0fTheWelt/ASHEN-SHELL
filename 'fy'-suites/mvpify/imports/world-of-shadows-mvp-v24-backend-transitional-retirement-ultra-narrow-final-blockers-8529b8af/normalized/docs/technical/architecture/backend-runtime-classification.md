...
## Ultra-narrow final blocker note

For the current package state, the remaining backend-local session-route residue is now concentrated in:

- POST bootstrap locality, and
- explicit compatibility fallback for snapshot/state/diagnostics when no authoritative story-session id is available.

This is narrower than a general local-session route surface and should be re-audited as such.

...
## Ultra-narrow final blocker addendum

This pass narrowed only the last remaining POST-locality and snapshot/state/diagnostics fallback residue.

### Additional narrowing achieved

- bound POST-created sessions now minimize local compatibility state immediately after successful authoritative binding;
- unbound fallback for `snapshot`, `state`, and `diagnostics` is no longer silent-by-default;
- those surfaces now require either authoritative story-session id or explicit compatibility opt-in (`allow_backend_local_fallback=1`).
...
- `session_store.py` still has intentional persistence/e2e readers.

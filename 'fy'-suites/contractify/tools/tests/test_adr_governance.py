...
def test_adr_governance_reports_legacy_inventory_and_canonical_targets() -> None:
    root = repo_paths.repo_root()
    payload = discover_adr_governance(root)
    assert payload["canonical_dir"] == "docs/ADR"
    assert payload["stats"]["n_adrs"] >= 3
    assert payload["stats"]["n_legacy_adrs"] >= 1
    assert any(f["kind"] == "legacy_adr_location" for f in payload["findings"])
...
def test_adr_governance_detects_canonical_path_collision(tmp_path: Path) -> None:
    ...
    (gov / "adr-0001-scene-identity-copy.md").write_text(

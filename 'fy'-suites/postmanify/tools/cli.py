...
def _cmd_plan(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({"openapi": str(openapi_path), "tag_folders": n_folders, "requests": n_reqs, "sub_suites": len(subs)}, indent=2))
    return 0
...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({"ok": True, "manifest": str((suites_dir.parent / "postmanify-manifest.json").relative_to(repo))}, indent=2))
    return 0
...
def main() -> int:
    ...
    p_gen.add_argument(
        "--out-master",
        default="postman/WorldOfShadows_Complete_OpenAPI.postman_collection.json",
        help="Repo-relative output path for the combined collection.",
    )
    p_gen.add_argument(
        "--suites-dir",
        default="postman/suites",

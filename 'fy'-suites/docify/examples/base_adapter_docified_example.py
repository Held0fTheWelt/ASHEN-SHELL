...
2. **Common lifecycle commands**
   ...
   ``self_audit``, ``release_readiness``, ``production_readiness``,
   ``import_bundle``, and ``consolidate``.
...
class BaseSuiteAdapter(ABC):
    ...
    for fix preparation, import handling, or consolidation.
    ...
    def self_governance_status(self) -> dict[str, Any]:
        ...
        -------
        dict[str, Any]
            A normalized governance payload containing:
            - overall ``ok`` status,
            - suite name,
            - blocking failures,
            - warnings,
            - workspace-level check results,
            - and suite-level check results.
        """
    ...
    def import_bundle(self, bundle_path: str, *, legacy: bool = False) -> dict[str, Any]:
        ...
        Suites that support importing current or legacy bundles should override
        this method. The base implementation keeps unsupported import work
        explicit instead of silently pretending success.
        """
        payload = {
            "ok": False,
            "suite": self.suite,

...
| **Open hotspots**                            | Top AST offenders still dense single-call orchestration: Writers Room `run_writers_room_packaging_stage` (~~319L), inspector `assemble_filled_inspector_sections` (~~167L), closure cockpit / evidence bundles (~~155–160L), `execute_users_update_put` (~~152L). Treated (DS-007 closed 2026-04-12): `run_validated_turn_pipeline` extracted `_log_narrative_outcomes` helper (staged pipeline), `narrative_threads_update_from_commit_impl` refactored with protocol documentation. Context-pack assembly: `rag_context_pack_`* modules + `assemble_context_pack` (DS-009 closed 2026-04-12). `ds005` clean; GoC solo builtin single template definition. |
...
| ------ | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------ |
| DS-006 | Long multi-step service orchestration in one callable | `writers_room_pipeline_packaging_stage.py` (~~319L), `writers_room_pipeline_generation_stage.py` (~~160L), `inspector_turn_projection_sections_assembly_filled.py` (~~167L), `ai_stack_closure_cockpit_report_assembly.py` / `ai_stack_evidence_session_bundle.py` (~~155–160L) | AST leaderboard; stage cohesion vs. file churn | Further stage-sized extractions or helper modules; keep public seams stable | High: many tests import service surfaces; one owner per wave |
...
| DS-008 | Improvement / recommendation single-function weight   | `improvement_service_recommendation_decision.py` (~158L), related improvement routes                                                                                                                                                                                            | Branch count inside decision application       | Extract policy/guard or step functions; preserve API contracts              | Medium: business rules drift if split carelessly             |
| DS-009 | RAG context-pack assembly (closed 2026-04-12)         | `rag_context_pack_section_titles` / `result_tail` / `compact_body` / `trace_footer` + `rag_context_pack_build` + assembler                                                                                                                                                      | Wave 1 + optional submodule split done         | —                                                                           | —                                                            |

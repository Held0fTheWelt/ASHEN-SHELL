...
Scans `despaghettify/superpowers/**/*.md` for:
  - Markdown link targets `](path)` (skips http(s), mailto, bare fragments)
  - Inline repo paths in backticks: `despaghettify/...`, `despaghettify/tools/...`, or `tools/...` with a file suffix
...
SCAN_ROOT = ROOT / "despaghettify" / "superpowers"

LINK_RE = re.compile(r"\]\(([^)]+)\)")
# Repo-like path inside backticks (file or trailing slash dir)
BACKTICK_REPO_RE = re.compile(
    r"`((?:despaghettify|despaghettify/tools|tools)/[a-zA-Z0-9_./-]+(?:\.(?:md|py|yaml|yml)|/))`"
)

...
after editing `despaghettify/superpowers/<name>/SKILL.md`. That command **copies** files into `.cursor/skills/` — identical behaviour on **Windows** and Unix.
...
- **@‑mention** (e.g. `@despaghettify/superpowers/despaghettify-orchestrate/SKILL.md`): useful for one-off context; it does **not** replace committed `.cursor/skills/` for discovery. If your branch is missing or stale under `.cursor/skills/`, run **`python despaghettify/tools/sync_despag_skills.py`** and commit the result — do not rely on @‑only workflows on Windows (or any OS).

## After pulling someone else’s branch

If `git pull` changed only `despaghettify/superpowers/` but not `.cursor/skills/` (merge conflict or partial update), run:

```bash
python despaghettify/tools/sync_despag_skills.py
```

Optional drift guard: `python despaghettify/tools/sync_despag_skills.py --check` (must exit **0** before push if your CI enforces it).
